...
void traverse_index_tree(Table &table, uint64_t page_no)
{
    ...
    if (page_header_size == 8)
    {
        traverse_index_leaf_page(table, file_offset, cell_count);
    }
    else if (page_header_size == 12)
    {
        auto child_pages = traverse_index_interior_page(table, file_offset, cell_count);
        // for (const auto &child : child_pages)
        // {
        //     // std::cout << "Traversing child page: " << child.left_child_page << std::endl;
        //     traverse_index_tree(table, child.left_child_page);
        // }
    }
    else
    {
